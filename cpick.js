...
proto.show = function() {
	...
	panel.style.top = pos.top - 140 + "px";
	panel.style.left = (pos.left + pos.width + 10) - 275 + "px";
...
proto._ok_clicked = function(e) {
    ...
    sendRgbColor(this.rgb); // 2010/11/28に追加。rgbの色を設置するため
};
...
// 2010/11/28 修正
    window._colorInit = _init; // input要素にカラーピッカー機能をつけるためグローバルに出す
    window.sendRgbColor = function(color) { // rgbのカラーを設置。changeが効かなかったので強引に
    var e = document.getElementById('rgbcolor');
    if(e) e.value = 'rgb(' + color['r'] + ', ' + color['g'] + ', ' + color['b'] + ')';
};
...
function _init(elempick) {
	var elms = /*_get_elements_by_class_name(document, "html5jp-cpick")*/[elempick];
	var n = 0;
	for( var i=0; i<elms.length; i++ ) {
		var elm = elms[i]/*.item(i)*/;
